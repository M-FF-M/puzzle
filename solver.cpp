...
bool rec_solve(bool find_all_sol, board& b, vector<piece>& pieces, map<int, int>& piece_map, int piece_idx = 0, int pos_idx = 0) {
  ...
  if (pos_idx == 0 && pieces[piece_idx].cpos.size() < pieces[piece_idx].pos.size())
  ...
  if (solvable) {
    solved = rec_solve(find_all_sol, b, pieces, piece_map, piece_idx + 1);
  } else {
  ...
  if (!solved || find_all_sol)
    return rec_solve(find_all_sol, b, pieces, piece_map, piece_idx, pos_idx + 1) || solved;
  else
    return solved;
}
...
void solve(bool find_all_sol, board& b, vector<piece>& pieces, map<int, int>& piece_map, string out_file) {
  ...
  cout << endl;
  bool foundsol = rec_solve(find_all_sol, b, pieces, piece_map);
